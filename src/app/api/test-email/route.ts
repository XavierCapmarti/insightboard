...
export async function GET(request: Request) {
  ...
  const result = await sendEmail({
    to,
    subject: '✅ ClarLens Email Test',
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #0ea5e9;">Email Configuration Working!</h1>
        <p>This is a test email from your ClarLens instance.</p>
        <p style="color: #64748b; font-size: 14px;">
          Sent at: ${new Date().toISOString()}
        </p>
        <hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;" />
        <p style="color: #94a3b8; font-size: 12px;">
          ClarLens - CSV to Insights
        </p>
      </div>
    `,
    text: `Email Configuration Working!\n\nThis is a test email from your ClarLens instance.\n\nSent at: ${new Date().toISOString()}`,
  });
